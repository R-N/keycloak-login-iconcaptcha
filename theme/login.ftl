            ...
            <#if realm.password>
                <form id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                    ...
                    <#if iconCaptchaRequired??>
                        <input type="hidden" name="_iconcaptcha-token" id="_iconcaptcha-token" />

                        <div class="iconcaptcha-holder" data-theme="${iconCaptchaTheme}"></div>
                        <link rel="stylesheet" href="${url.resourcesPath}/iconcaptcha/css/icon-captcha.min.css" />
                        <link rel="stylesheet" href="${iconCaptchaFrontServerUrl}/assets/css/icon-captcha.min.css" />
                        <script src="${url.resourcesPath}/iconcaptcha/js/icon-captcha.min.js"></script>
                        <script src="${iconCaptchaFrontServerUrl}/assets/js/icon-captcha.min.js"></script>
                        <script>
                            document.addEventListener('DOMContentLoaded', function () {
                                IconCaptcha.init('.iconcaptcha-holder', {
                                    general: {
                                        validationPath: '${iconCaptchaFrontServerUrl}/src/captcha-request.php'
                                    }
                                });
                                ...
                                IconCaptcha.init('.iconcaptcha-holder', {
                                    ...
                                    messages: {
                                        initialization: {
                                            verify: 'Verifikasi Anda bukan robot.',
                                            loading: 'Loading...',
                                        },
                                        header: 'Pilih gambar yang <u>paling sedikit</u> muncul',
                                        correct: 'Verifikasi berhasil.',
                                        incorrect: {
                                            title: 'Salah',
                                            subtitle: "Anda memilih gambar yang salah."
                                        },
                                        timeout: {
                                            title: 'Mohon tunggu 60 detik.',
                                            subtitle: 'Anda terlalu banyak salah.'
                                        }
                                    }
                                })
                            });
